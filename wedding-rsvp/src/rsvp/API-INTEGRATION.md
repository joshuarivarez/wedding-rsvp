...
Save one response per invitation, updating it on subsequent submissions. Return errors without resolving the receipt so the form can preserve selections and offer retry. Optional note fields are limited to 500 characters each for diet/allergies, 150 for song requests, and 1000 for the message.
